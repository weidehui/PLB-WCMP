...
std::stringstream tlbBibleFilename;  //用来记录tlb路径信息
std::stringstream tlbBibleFilename2;  //用来记录tlb路径的变化信息
...
double poission_gen_interval(double avg_rate) //泊松？
{
    if (avg_rate > 0)
...
template<typename T> 
T rand_range (T min, T max)  //随机数
{
    return min + ((double)max - min) * rand () / RAND_MAX;
}

//Dehui
void install_applications (int fromLeafId, NodeContainer top_servers, NodeContainer servers, double requestRate, struct cdf_table *cdfTable,
        long &flowCount, long &totalFlowSize, int SERVER_COUNT, int LEAF_COUNT, double START_TIME, double END_TIME, double FLOW_LAUNCH_END_TIME, uint32_t applicationPauseThresh, uint32_t applicationPauseTime)
{
    ...
    {
        int fromServerIndex = fromLeafId * SERVER_COUNT + i;//第几个server

        double startTime = START_TIME + poission_gen_interval (requestRate);//开始时间
        while (startTime < FLOW_LAUNCH_END_TIME) //如果没到结束时间
        {
            flowCount ++;
            ...
            // Install apps
            ApplicationContainer sourceApp = source.Install (top_servers.Get (fromServerIndex));
...
//Dehui
void PrintRoute (NodeContainer* SpecificNode,string typeName)
{
  Ipv4GlobalRoutingHelper ipv4Route;
  //cout<<"PrintRoute"<<endl;
  for (uint32_t i=0;i<SpecificNode->GetN();i++)
  {
    stringstream filename;
    filename << typeName << "-" << i << ".routes";
    Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> (filename.str(),std::ios::out);
    ipv4Route.PrintRoutingTableEvery(Seconds(1),SpecificNode->Get(i),routingStream);
    //ipv4Route.PrintRoutingTableAt(Seconds(interval),SpecificNode->Get(i),routingStream);
    //ipv4Route.PrintRoutingTableAllEvery(Seconds(interval),routingStream);
    //ipv4Route.PrintRoutingTableAllAt(Seconds(interval),routingStream);
  }
}
...
int main (int argc, char *argv[])
{
    ...
    uint32_t linkLatency = 10; //MicroSeconds微妙
    ...
    spines.Create (SPINE_COUNT);  //上一层层AR
    NodeContainer leaves;
    leaves.Create (LEAF_COUNT); //下一层AR
    ...
    NodeContainer CRs;
    CRs.Create(CR_COUNT);
    NodeContainer top_servers;
    top_servers.Create (SERVER_COUNT * CR_COUNT);
    //Dehuio
    ...
    else if (runMode == TLB)
    {
        ...
        internet.Install (leaves);
        //Dehui
        internet.Install (BRs);
        internet.Install (CRs);
    }
    ...
    else if (runMode == ECMP || runMode == FlowBender)
    {
        ...
        cout<<"runMode == ECMP,SetRoutingHelper (globalRoutingHelper)"<<endl;
    ...
    NS_LOG_INFO ("Configuring servers");   //安装server
    // Setting servers
    ...
    std::vector<Ipv4Address> leafNetworks (LEAF_COUNT + CR_COUNT);//Dehui

    std::vector<Ipv4Address> serverAddresses (SERVER_COUNT * LEAF_COUNT + SERVER_COUNT * CR_COUNT); //Dehui

    std::map<std::pair<int, int>, uint32_t> leafToSpinePath;
    std::map<std::pair<int, int>, uint32_t> spineToLeafPath;
    //Dehui
    std::map<std::pair<int, int>, uint32_t> spineToBRPath;
    std::map<std::pair<int, int>, uint32_t> BRToSpinePath;
    std::map<std::pair<int, int>, uint32_t> BRToCRPath;
    std::map<std::pair<int, int>, uint32_t> CRToBRPath;
    //Dehuio

    std::vector<Ptr<Ipv4TLBProbing> > probings (SERVER_COUNT * CR_COUNT); //Dehui
    ...
    for (int i = 0; i < LEAF_COUNT; i++)
    {
        ...
        for (int j = 0; j < SERVER_COUNT; j++)
        {
            ...
            if (runMode == TLB)
            {
                for (int k = 0; k < SERVER_COUNT * LEAF_COUNT; k++)     //安装的server数量，默认一个上面8个server，4个Leaf
                {
                ...
                for (int k = 0; k < SERVER_COUNT * CR_COUNT; k++)     //安装的server数量，默认一个上面8个server，4个Leaf
                {
                    Ptr<Ipv4TLB> tlb = top_servers.Get (k)->GetObject<Ipv4TLB> ();
                    tlb->AddAddressWithTor (interfaceContainer.GetAddress (1), i);
                    // NS_LOG_INFO ("Configuring TLB with " << k << "'s server, inserting server: " << j << " under leaf: " << i);
                }
            }
    ...
    {
        Ipv4Address network = ipv4.NewNetwork ();
        leafNetworks[LEAF_COUNT+i] = network;

        for (int j = 0; j < SERVER_COUNT; j++)
        {
            int serverIndex = i * SERVER_COUNT + j;
            NodeContainer nodeContainer = NodeContainer (CRs.Get (i), top_servers.Get (serverIndex));
            NetDeviceContainer netDeviceContainer = p2p.Install (nodeContainer);
            ...
            NS_LOG_INFO ("CR - " << i << " is connected to Server - " << j << " with address "
                    << interfaceContainer.GetAddress(0) << " <-> " << interfaceContainer.GetAddress (1)
                    << " with port " << netDeviceContainer.Get (0)->GetIfIndex () << " <-> " << netDeviceContainer.Get (1)->GetIfIndex ());

            serverAddresses [SERVER_COUNT * LEAF_COUNT+serverIndex] = interfaceContainer.GetAddress (1);
            if (transportProt.compare ("Tcp") == 0)
            {
                tc.Uninstall (netDeviceContainer);
            }
            ...
            if (runMode == TLB)
            {
                ...
                {
                    Ptr<Ipv4TLB> tlb = servers.Get (k)->GetObject<Ipv4TLB> ();
                    tlb->AddAddressWithTor (interfaceContainer.GetAddress (1), i+LEAF_COUNT);
                    // NS_LOG_INFO ("Configuring TLB with " << k << "'s server, inserting server: " << j << " under leaf: " << i);
                }
                for (int k = 0; k < SERVER_COUNT * CR_COUNT; k++)     //安装的server数量，默认一个上面8个server，4个Leaf
                {
                    Ptr<Ipv4TLB> tlb = top_servers.Get (k)->GetObject<Ipv4TLB> ();
                    tlb->AddAddressWithTor (interfaceContainer.GetAddress (1), i+LEAF_COUNT);  //Dehui 这里地址改了
                    // NS_LOG_INFO ("Configuring TLB with " << k << "'s server, inserting server: " << j << " under leaf: " << i);
                }
            }
    ...
    std::set<std::pair<uint32_t, uint32_t> > asymLink; // set< (A, B) > Leaf A -> Spine B is asymmetric  非对称
        ...
        for (int j = 0; j < SPINE_COUNT; j++)
        {
            ...
            for (int l = 0; l < LINK_COUNT; l++)
            {
                ...
                if (runMode == TLB || runMode == DRB || runMode == PRESTO || runMode == WEIGHTED_PRESTO || runMode == Clove)
                {
                    ...
                    leafToSpinePath[leafToSpine] = netDeviceContainer.Get (0)->GetIfIndex (); //获取端口
        ...
        for (int j = 0; j < BR_COUNT; j++)
        {
            ...
            if(i==j)
            {
                for (int l = 0; l < LINK_COUNT; l++)
                {
                    ...
                    NodeContainer nodeContainer = NodeContainer (spines.Get (i), BRs.Get (j));
                    NetDeviceContainer netDeviceContainer = p2p.Install (nodeContainer);
                    if (transportProt.compare ("DcTcp") == 0)
                    {
                        NS_LOG_INFO ("Install RED Queue for leaf: " << i << " and spine: " << j);
                    ...
                    Ipv4InterfaceContainer ipv4InterfaceContainer = ipv4.Assign (netDeviceContainer);
                    NS_LOG_INFO ("spine - " << i << " is connected to BR - " << j << " with address "
                            << ipv4InterfaceContainer.GetAddress(0) << " <-> " << ipv4InterfaceContainer.GetAddress (1)
                            << " with port " << netDeviceContainer.Get (0)->GetIfIndex () << " <-> " << netDeviceContainer.Get (1)->GetIfIndex ()
                            << " with data rate " << spineLeafCapacity);
                    ...
                    {
                        //Dehui
                        std::pair<int, int> spineToBR = std::make_pair(i, j);
                        spineToBRPath[spineToBR] = netDeviceContainer.Get (0)->GetIfIndex ();

                        std::pair<int, int> BRToSpine = std::make_pair(j, i);
                        BRToSpinePath[BRToSpine] = netDeviceContainer.Get (1)->GetIfIndex ();
                    }
    ...
    for (int i = 0; i < BR_COUNT; i++)
    {
        ...
        for (int j = 0; j < CR_COUNT; j++)
        {
            for (int l = 0; l < LINK_COUNT; l++)
            {
                ...
                NodeContainer nodeContainer = NodeContainer (BRs.Get (i), CRs.Get (j));
                NetDeviceContainer netDeviceContainer = p2p.Install (nodeContainer);
                if (transportProt.compare ("DcTcp") == 0)
                {
                    NS_LOG_INFO ("Install RED Queue for leaf: " << i << " and spine: " << j);
                ...
                Ipv4InterfaceContainer ipv4InterfaceContainer = ipv4.Assign (netDeviceContainer);
                NS_LOG_INFO ("BR - " << i << " is connected to CR - " << j << " with address "
                             << ipv4InterfaceContainer.GetAddress(0) << " <-> " << ipv4InterfaceContainer.GetAddress (1)
                             << " with port " << netDeviceContainer.Get (0)->GetIfIndex () << " <-> " << netDeviceContainer.Get (1)->GetIfIndex ()
                             << " with data rate " << spineLeafCapacity);
                ...
                if (runMode == TLB || runMode == DRB || runMode == PRESTO || runMode == WEIGHTED_PRESTO || runMode == Clove)
                {
                    std::pair<int, int> BRToCR = std::make_pair(i, j);
                    BRToCRPath[BRToCR] = netDeviceContainer.Get (0)->GetIfIndex ();

                    std::pair<int, int> CRToBR = std::make_pair(j, i);
                    CRToBRPath[CRToBR] = netDeviceContainer.Get (1)->GetIfIndex ();
                }
...
//     filename << typeName << "-" << i << ".routes";
//     Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> (filename.str(),std::ios::out);
//     ipv4Route.PrintRoutingTableEvery(Seconds(interval),SpecificNode->Get(i),routingStream);
//     //ipv4Route.PrintRoutingTableAt(Seconds(interval),SpecificNode->Get(i),routingStream);
//     //ipv4Route.PrintRoutingTableAllEvery(Seconds(interval),routingStream);
//     //ipv4Route.PrintRoutingTableAllAt(Seconds(interval),routingStream);
//   }
// }
    if (runMode == ECMP || runMode == PRESTO || runMode == WEIGHTED_PRESTO || runMode == DRB || runMode == FlowBender || runMode == TLB || runMode == Clove)
    {
        ...
        Ipv4GlobalRoutingHelper::PopulateRoutingTables (); //计算路由表
       // cout<<"计算路由表"<<endl;
        // string sp = "spine";
        // PrintRoute(&spines,sp);
        // string le = "leaf";
        // PrintRoute(&leaves,le);
        // string cr="CR";
        // PrintRoute(&CRs,cr);
        // string br="BR";
        // PrintRoute(&BRs,br);
        // string ts="top_servers";
        // PrintRoute(&top_servers,ts);
        // string ls="Leaf_servers";
        // PrintRoute(&servers,ls);
    ...
    if (runMode == TLB)
    {
        NS_LOG_INFO ("Configuring TLB available paths");//配置TLB的可达路径
        // for (int i = 0; i < LEAF_COUNT; i++)
        // {
        //     for (int j = 0; j < SERVER_COUNT; j++)
        //     {
        ...
        //                 tlb->AddAvailPath (l, newPath); //l是目的Tor
        ...
        for (int i = 0; i < CR_COUNT; i++)
        {
            ...
            {
                int serverIndex = i * SERVER_COUNT + s;
                for (int j = 0; j < BR_COUNT; j++)
                {
                    int path = 0;
                    int pathBase = 1;
                    path += CRToBRPath[std::make_pair(i, j)] * pathBase;
                    pathBase *= 100;
                    for (int k = 0; k < SPINE_COUNT; k++)
                    {
                        if (j == k)
                        {
                            path = BRToSpinePath[std::make_pair(j, k)] * pathBase + path;
                            pathBase *= 1000;
                            for (int l = 0; l < LEAF_COUNT; l++)
                            {
                                int newPath = spineToLeafPath[std::make_pair(k, l)] * pathBase + path;
                                Ptr<Ipv4TLB> tlb = top_servers.Get (serverIndex)->GetObject<Ipv4TLB> ();
                                tlb->AddAvailPath (l, newPath); //l是目的Tor
        ...
        if (runMode == TLB && TLBProbingEnable) //探针配置
        {
            ...
            //     int serverIndexUnderLeaf = i % SERVER_COUNT; //本leaf节点下的第几个

            //     if (serverIndexUnderLeaf < LEAF_COUNT) //如果小于leafcount
            //     {
            //         int serverBeingProbed = SERVER_COUNT * serverIndexUnderLeaf; //也去往另外的
            ...
            //Dehui
            //配置探针
            for (int i = 0; i < CR_COUNT*SERVER_COUNT; i++)
            {
                
                // The i th server under one leaf is used to probe the leaf i by contacting the i th server under that leaf
                Ptr<Ipv4TLBProbing> probing = CreateObject<Ipv4TLBProbing> ();
                probings[i] = probing;
                probing->SetNode (top_servers.Get (i));
                probing->SetSourceAddress (serverAddresses[LEAF_COUNT*SERVER_COUNT+ i]);
                probing->Init ();
                ...
                if (serverIndexUnderLeaf < LEAF_COUNT) //如果小于leafcount
                {
                    ...
                    probing->SetProbeAddress (serverAddresses[serverBeingProbed]);
                    //NS_LOG_INFO ("Server: " << i << " is going to probe server: " << serverBeingProbed);
                    int leafIndex = i / SERVER_COUNT; //第几个leaf
                    for (int j = leafIndex * SERVER_COUNT; j < leafIndex * SERVER_COUNT + SERVER_COUNT; j++)
                    {
                        if (i == j)
                        {
                        ...
                        probing->AddBroadCastAddress (serverAddresses[LEAF_COUNT*SERVER_COUNT+j]);
                        //NS_LOG_INFO ("Server:" << i << " is going to broadcast to server: " << j);
                    }
                    probing->StartProbe ();
                    probing->StopProbe (Seconds (END_TIME));
    ...
    NS_LOG_INFO ("Calculating request rate");  //计算需要的rate
    ...
    for (int fromLeafId = 0; fromLeafId < CR_COUNT; fromLeafId ++)
    {
        install_applications(fromLeafId, top_servers, servers, requestRate, cdfTable, flowCount, totalFlowSize, SERVER_COUNT, LEAF_COUNT, START_TIME, END_TIME, FLOW_LAUNCH_END_TIME, applicationPauseThresh, applicationPauseTime);
    }
    ...
    std::stringstream flowMonitorFilename; //流的监视文件
    std::stringstream linkMonitorFilename;  //链路的监视文件
    ...
    flowMonitorFilename << "b" << BUFFER_SIZE << ".xml";   //xml是流的情况
    linkMonitorFilename << "b" << BUFFER_SIZE << "-link-utility.out";  //out是链路的情况
