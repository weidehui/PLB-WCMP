...
public:
  ...
  struct FlowStats
  {
    ...
    /// reception starts。 流中第一个包被end node收到的时间
    ...
    /// Contains the absolute time when the last packet in the flow
    /// was received, i.e. the time when the flow reception ends。  流中最后一个包被收到的时间
    Time     timeLastRxPacket;

    /// Contains the sum of all end-to-end delays for all received
    /// packets of the flow.   所有延迟的时间
    Time     delaySum; // delayCount == rxPackets
  
    /// Contains the sum of all end-to-end delay jitter (delay   延迟抖动
