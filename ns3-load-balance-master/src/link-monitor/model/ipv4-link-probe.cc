     ...
     m_checkTime (MicroSeconds (100))
{
  ...
  // Notice, the interface at 0 is loopback, we simply ignore it  忽略loopback 0号交换机
  for (uint32_t interface = 1; interface < m_ipv4->GetNInterfaces (); ++interface)
  {
    ...
    m_queueProbe[interface] = Create<Ipv4QueueProbe> (); //创建了一个探针
    m_queueProbe[interface]->SetInterfaceId (interface); //设置端口号
    m_queueProbe[interface]->SetIpv4LinkProbe (this);  //探针和端口绑定
...
Ipv4LinkProbe::CheckCurrentStatus ()
{
  for (uint32_t interface = 1; interface < m_ipv4->GetNInterfaces (); ++interface)
  {
    ...
    if (itr == m_stats.end ())
    {
      ...
      newStats.checkTime = Simulator::Now ();  //时间
    ...
    else
    {
      ...
      newStats.accumulatedTxBytes = m_accumulatedTxBytes[interface];
      newStats.txLinkUtility =
          Ipv4LinkProbe::GetLinkUtility (interface, m_accumulatedTxBytes[interface] - lastTxBytes, m_checkTime); //每100ms测一次
      newStats.accumulatedDequeueBytes = m_accumulatedDequeueBytes[interface];
      newStats.dequeueLinkUtility =
          Ipv4LinkProbe::GetLinkUtility (interface, m_accumulatedDequeueBytes[interface] - lastDequeueBytes, m_checkTime);
      newStats.packetsInQueue = m_NPacketsInQueue[interface];
...
double
Ipv4LinkProbe::GetLinkUtility (uint32_t interface, uint64_t bytes, Time time)  //链路利用率
{
