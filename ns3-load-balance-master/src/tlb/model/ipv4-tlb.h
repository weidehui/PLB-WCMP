...
public:
    ...
    //Dehui
    uint32_t GetPath (uint32_t size, uint32_t flowId, Ipv4Address saddr, Ipv4Address daddr);
    ...
    //Dehui
    uint32_t GetDre (Ipv4Address daddr, uint32_t path);
    void SetRemoteDre(Ipv4Address daddr, uint32_t path,uint32_t dre);
...
private:
    ...
    //Dehui
    bool WhereToChange (uint32_t size, uint32_t destTor, PathInfo &newPath, bool hasOldPath, uint32_t oldPath);
    ...
    */
    ...
    //Dehui
    double m_rttweightAlpha;
    double m_lossweightBeta;
