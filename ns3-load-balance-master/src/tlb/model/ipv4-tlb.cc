...
Ipv4TLB::Ipv4TLB ():
    ...
    m_agingCheckTime (MicroSeconds (1000)),
    m_dreTime (MicroSeconds (200)),
    m_dreAlpha (0.2),
    m_dreDataRate (DataRate ("1Gbps")),
    m_dreQ (3),
    m_dreMultiply (5),
    m_minRtt (MicroSeconds (100)), // 0.1ms
    ...
    */
    ...
    m_rttAlpha(1.0),
    m_ecnBeta(0.0),
    m_rttweightAlpha(0.5),
    m_lossweightBeta(0.5)
{
...
Ipv4TLB::Ipv4TLB (const Ipv4TLB &other):
    ...
    */
    ...
    m_rttweightAlpha(other.m_rttweightAlpha),
    m_lossweightBeta(other.m_lossweightBeta)
{
    NS_LOG_FUNCTION (this);
}
...
Ipv4TLB::GetTypeId (void)
{
    static TypeId tid = TypeId ("ns3::Ipv4TLB")
      ...
      .AddAttribute ("ECNBeta",
        ...
        //Dehui
      .AddAttribute ("RttweightAlpha",
                     "They weight of path RTT",
                     DoubleValue (0.5),
                     MakeDoubleAccessor (&Ipv4TLB::m_rttweightAlpha),
                     MakeDoubleChecker<double> ())

      .AddAttribute ("LossweightBeta",
                     "They weight of path lossrate",
                     DoubleValue (0.5),
                     MakeDoubleAccessor (&Ipv4TLB::m_lossweightBeta),
                     MakeDoubleChecker<double> ())
...
Ipv4TLB::AddAddressWithTor (Ipv4Address address, uint32_t torId)
{
    ...
    std::cout<<"Ipv4Address address:"<<address<<" torid:"<<torId<<std::endl;
}
...
{
    std::cout<<"3333333GetPath:"<<std::endl;
    ...
    uint32_t destTor = 0;
    if (!Ipv4TLB::FindTorId (daddr, destTor)) //先找寻目的tor
    {
        NS_LOG_ERROR ("Cannot find dest tor id based on the given dest address");
        return 0;
    }
    ...
    if (!Ipv4TLB::FindTorId (saddr, sourceTor))//找寻源tor
    {
    ...
    {
        // New flow
        std::cout<<"3333333NewFlow:"<<std::endl;
        struct PathInfo newPath;
        if (Ipv4TLB::WhereToChange (destTor, newPath, false, 0))
        {
...
bool
Ipv4TLB::WhereToChange (uint32_t size, uint32_t destTor, PathInfo &newPath, bool hasOldPath, uint32_t oldPath)
{
    std::map<uint32_t, std::vector<uint32_t> >::iterator itr = m_availablePath.find (destTor);
    std::cout<<"222222WhereToChange :"<<std::endl;
    ...
    for ( ; vectorItr != (itr->second).end (); ++vectorItr)
    {
        ...
        std::map<std::pair<uint32_t, uint32_t>, TLBPathInfo>::iterator itr = m_pathInfo.find (key); //找到对应的路径
        TLBPathInfo pathInfo = itr->second;
        rate +=pathInfo.size;
        rate +=pathInfo.sizeremote;
    }
    double pathseed=rand()%1;
    double pathweight=rate*pathseed;
    for ( ; vectorItr != (itr->second).end (); ++vectorItr)
    {
        uint32_t pathId = *vectorItr;
        //struct PathInfo pathInfo = JudgePath (destTor, pathId);
        ...
        struct PathInfo path = JudgePath (destTor, pathId); //判断路径
       // std::cout<<"11111111path.quantifiedDre:"<<path.quantifiedDre<<std::endl;
        path.pathId = pathId;
        ...
        TLBPathInfo pathInfo = itr->second;
        uint32_t weight=pathInfo.size+pathInfo.sizeremote;
...
uint32_t
Ipv4TLB::GetPath (uint32_t size , uint32_t flowId, Ipv4Address saddr, Ipv4Address daddr)
{
    std::cout<<"1111:GetPath (uint32_t size , uint32_t flowId, Ipv4Address saddr, Ipv4Address daddr):"<<std::endl;
    if (!m_agingEvent.IsRunning ())//复位判断
    {
        m_agingEvent = Simulator::Schedule (m_agingCheckTime, &Ipv4TLB::PathAging, this);
    }

    if (!m_dreEvent.IsRunning ())  //dre复位判断
    {
        m_dreEvent = Simulator::Schedule (m_dreTime, &Ipv4TLB::DreAging, this);
    }
    ...
    if (!Ipv4TLB::FindTorId (daddr, destTor)) //先找寻目的tor
    {
        std::cout<<"22222:NotFinddestTor (saddr, sourceTor))::"<< daddr<<std::endl;   
        NS_LOG_ERROR ("Cannot find dest tor id based on the given dest address");
        return 0;
    }
    std::cout<<"1111:FindTorId (saddr, sourceTor)): "<< destTor<<std::endl;

    uint32_t sourceTor = 0;
    //std::cout<<"1111:FindTorId (saddr, sourceTor))"<<std::endl;
    if (!Ipv4TLB::FindTorId (saddr, sourceTor))//找寻源tor
    {
        std::cout<<"1111:NotFindTorId (saddr, sourceTor))"<<std::endl;
        NS_LOG_ERROR ("Cannot find source tor id based on the given source address");
    }
    std::cout<<"1111:FindTorId (saddr, sourceTor)): "<< sourceTor<<std::endl;

    std::map<uint32_t, TLBFlowInfo>::iterator flowItr = m_flowInfo.find (flowId);  //找到这个flow
    ...
    if (flowItr == m_flowInfo.end ())
    {
        // New flow
        struct PathInfo newPath;
       // std::cout<<"1111:New flow"<<std::endl;
        if (Ipv4TLB::WhereToChange (size, destTor, newPath, false, 0))
        {
            std::cout<<"1111:WhereToChange:"<<std::endl;
            std::ofstream outfile;
            outfile.open("tlblog.txt",std::ios::app);
            outfile<<"WhereToChange:"<<std::endl;
            outfile.close();
            m_pathSelectTrace (flowId, sourceTor, destTor, newPath.pathId, false, newPath, Ipv4TLB::GatherParallelPaths (destTor));
        }
        else
        {
            std::ofstream outfile;
            outfile.open("tlblog.txt",std::ios::app);
            outfile<<"SelectRandomPath:"<<std::endl;
            outfile.close();
            newPath = Ipv4TLB::SelectRandomPath (destTor);
            m_pathSelectTrace (flowId, sourceTor, destTor, newPath.pathId, true, newPath, Ipv4TLB::GatherParallelPaths (destTor));
        }
    ...
    // {
    //     std::ofstream outfile;
    //         outfile.open("tlblog.txt",ios::app);
    //         outfile<<"else if (m_rerouteEnable):"<<std::endl;
    //         outfile.close();
    //     Time flowActiveTime = (flowItr->second).activeTime;
    //     (flowItr->second).activeTime = Simulator::Now ();
    ...
    //     // Old flow
    //     uint32_t oldPath = (flowItr->second).path;
    //     std::pair<uint32_t, uint32_t> key = std::make_pair (destTor, oldPath);
    //     std::map<std::pair<uint32_t, uint32_t>, TLBPathInfo>::iterator itr = m_pathInfo.find (key);
    //     TLBPathInfo pathInfo = itr->second;
    //     pathInfo.thelosssize +=size;
    //     struct PathInfo oldPathInfo = Ipv4TLB::JudgePath (destTor, oldPath);
    //     // if (m_respondToFailure
    //     //         && ((flowItr->second).retransmissionSize > m_flowRetransVeryHigh
    //     //         || (flowItr->second).timeoutCount >= 1))
    //     // {
    //         struct PathInfo newPath;
    //         if (Ipv4TLB::WhereToChange (size,destTor, newPath, true, oldPath))
    //         {
    //             if (newPath.pathId != oldPath)
    //             {
    //                 m_pathChangeTrace (flowId, sourceTor, destTor, newPath.pathId, oldPath, false, Ipv4TLB::GatherParallelPaths (destTor));
    ...
    else
    {
        // std::ofstream outfile;
        //     outfile.open("tlblog.txt",std::ios::app);
        //     outfile<<"else oldPath:"<<std::endl;
        //     outfile.close();
        //std::cout<<"1111:OldFlow"<<std::endl;
        (flowItr->second).activeTime = Simulator::Now ();
...
Ipv4TLB::FlowSend (uint32_t flowId, Ipv4Address daddr, uint32_t path, uint32_t size, bool isRetransmission)
{
    ...
    {
        //Dehui
        std::pair<uint32_t, uint32_t> key = std::make_pair(destTor, path);//目的地和path相连
        std::map<std::pair<uint32_t, uint32_t>, TLBPathInfo>::iterator itr = m_pathInfo.find (key);//找到路径的信息
...
//Dehui
uint32_t
Ipv4TLB::GetDre(Ipv4Address daddr, uint32_t path)
{
    uint32_t destTor = 0;
    Ipv4TLB::FindTorId (daddr, destTor);
    std::pair<uint32_t, uint32_t> key = std::make_pair(destTor, path);
    std::map<std::pair<uint32_t, uint32_t>, TLBPathInfo>::iterator itr = m_pathInfo.find (key);
    TLBPathInfo pathInfo;
    pathInfo = itr->second;
    //std::cout<<"666666:GetDre:"<<pathInfo.size<<" PathId:"<< pathInfo.pathId<<std::endl;
    return pathInfo.size;
}
void
Ipv4TLB::SetRemoteDre(Ipv4Address daddr, uint32_t path,uint32_t dre)
{
    uint32_t destTor = 0;
    Ipv4TLB::FindTorId (daddr, destTor);
    std::pair<uint32_t, uint32_t> key = std::make_pair(destTor, path);
    ...
    TLBPathInfo pathInfo;
    pathInfo = itr->second;
    pathInfo.sizeremote=dre;
    //std::cout<<"141441414:pathInfo.sizeremote:"<<pathInfo.sizeremote<<std::endl;
}
...
Ipv4TLB::UpdatePathInfo (uint32_t destTor, uint32_t path, uint32_t size, bool withECN, Time rtt)
{
    ...
    //Dehui
    //pathInfo.pathweight=(double)1/(m_rttweightAlpha*rtt.GetSeconds()+m_lossweightBeta*pathInfo.lossrate);
    pathInfo.rtt=rtt;
    // Added Jan 11st
    /*
...
Ipv4TLB::SendPath (uint32_t destTor, uint32_t path, uint32_t size)
{
    ...
    // (itr->second).unitysize +=size;
    //std::cout<<"666666666(itr->second).dreValue:"<<(itr->second).dreValue<<" (itr->second).unitysize +=size;"<<(itr->second).unitysize<<std::endl;
}
...
Ipv4TLB::GetInitPathInfo (uint32_t path)
{
    ...
    pathInfo.thelosssize=0;
    pathInfo.rtt=Seconds(0);
    //pathInfo.unitysize=0;
    pathInfo.sizeremote=0;
...
{
    std::cout<<"33333333333WhereToChange "<<std::endl;  
    std::map<uint32_t, std::vector<uint32_t> >::iterator itr = m_availablePath.find (destTor);
...
Ipv4TLB::JudgePath (uint32_t destTor, uint32_t pathId)
{
    ...
    std::cout<<"55555555QuantifyDre (pathInfo.dreValue);:"<<path.quantifiedDre<<std::endl;
...
Ipv4TLB::DreAging (void)
{
    ...
    for ( ; itr != m_pathInfo.end (); ++itr)
    {
        ...
        //Dehui
         //(itr->second).unitysize *= (1 - m_dreAlpha);
    }
