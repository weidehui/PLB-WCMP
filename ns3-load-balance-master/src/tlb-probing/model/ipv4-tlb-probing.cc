...
Ipv4TLBProbing::SendProbe (uint32_t path)
{
    ...
    uint32_t drev=ipv4TLB->GetDre(m_probeAddress, path);
    packet->AddDre(drev);
    //std::cout<<"12121212:"<<drev<<std::endl;
...
Ipv4TLBProbing::ReceivePacket (Ptr<Socket> socket)
{
    ...
    else
    {
        ...
        //Dehui
        uint32_t dreremote=packet->GetDre();
        //std::cout<<"131311313SetRemoteDre:"<<dreremote<<std::endl;
        ipv4TLB->SetRemoteDre(probingTag.GetProbeAddres (),path,dreremote);
        if (!probingTag.GetIsBroadcast ())
        {
