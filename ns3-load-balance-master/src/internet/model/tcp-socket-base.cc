...
TcpSocketBase::DoConnect (void)
{
  ...
  if (m_state == CLOSED || m_state == LISTEN || m_state == SYN_SENT || m_state == LAST_ACK || m_state == CLOSE_WAIT)
    { // send a SYN packet and change state into SYN_SENT
      ...
      if (m_TLBEnabled)
      {
          std::cout<<"m_TLBSendSide = true;"<<std::endl;
          m_TLBSendSide = true;
      }
      if (m_CloveEnabled)
      {
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  SequenceNumber32 ackNumber = tcpHeader.GetAckNumber ();  //获取acknumber
  uint32_t bytesAcked = ackNumber - m_txBuffer->HeadSequence ();  //收到的ack-发出去的 是被确认的字节
  uint32_t segsAcked  = bytesAcked / m_tcb->m_segmentSize;  //确认了几个段
...
TcpSocketBase::SendEmptyPacket (uint8_t flags)
{
  ...
  if (m_TLBEnabled)
  {
    if (m_TLBSendSide)
    {
      std::cout<<"SendEmptyPacket:f (m_TLBSendSide): "<<std::endl;
      uint32_t flowId = TcpSocketBase::CalFlowId (m_endPoint->GetLocalAddress (),
                         m_endPoint->GetPeerAddress (), header.GetSourcePort (), header.GetDestinationPort ());

      Ptr<Ipv4TLB> ipv4TLB = m_node->GetObject<Ipv4TLB> ();
      uint32_t path = ipv4TLB->GetPath (p->GetSize (),flowId, m_endPoint->GetLocalAddress (), m_endPoint->GetPeerAddress ());
  ...
  if (m_endPoint)
    {
      ...
      if (m_TLBEnabled && m_TLBSendSide)
      {
        std::cout<<"SendDataPacket:"<<std::endl;
        uint32_t flowId = TcpSocketBase::CalFlowId (m_endPoint->GetLocalAddress (),
                         m_endPoint->GetPeerAddress (), header.GetSourcePort (), header.GetDestinationPort ());
        Ptr<Ipv4TLB> ipv4TLB = m_node->GetObject<Ipv4TLB> ();
        //Dehui
  ...
  while (m_txBuffer->SizeFromSequence (m_nextTxSequence))
    {
      ...
      // m_lossrate=1-(double)sz/s;  //计算丢包率
      // //if TLB support
      // if (m_TLBEnabled && m_TLBSendSide)
      // {
      //   TcpTLBTag tcpTLBTag;
      ...
      //   ipv4TLB->UpdatePathInfo(path, m_endPoint->GetLocalAddress (), m_lossrate);
      // }
